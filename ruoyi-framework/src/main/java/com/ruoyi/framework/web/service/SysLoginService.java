...
public class SysLoginService {
    ...
    public String emailLogin(String email, String emailCode) {
        if (!(StringUtils.isNotEmpty(email) && email.matches("\\w{1,30}@[a-zA-Z0-9]{2,20}(\\.[a-zA-Z0-9]{2,20}){1,2}"))) {
            throw new RuntimeException("邮箱为空或邮箱格式不合法");
        }
        EmailCodeBody emailCodeBody = new EmailCodeBody();
        emailCodeBody.setEmail(email);
        emailCodeBody.setCode(emailCode);
        AjaxResult ajaxResult = iSysUserService.judgmentEmailCode(emailCodeBody);
        if(ajaxResult.isError()){
            throw new RuntimeException(ajaxResult.get(MSG_TAG).toString());
        }
        // 用户验证
        Authentication authentication;
        try {
